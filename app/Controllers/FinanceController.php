...
class FinanceController extends ResourceController
{
    ...
    public function transfer()
    {
        ...
        $amount = $data->amount;
        if ($amount <= 0) return $this->jsonResponse->error("Amount must be positive", 400);
        ...
        try {
            ...
            // Use '1099' or similar as Clearing Account if needed, or direct transfer logic if simpler.
            ...
            // Let's use a "Suspense/Clearing" account '1099' for checks.
            ...
            // We need an "Internal Transfer" account. Let's assume '1099' is Funds in Transit (Asset).
            ...
            // Let's implement as:
            // 1. Source Journal: Cr SourceAccount, Dr '1099' (Funds in Transit).
            $this->addJournalItem($j1, $data->source_account_code, 0, $amount); // Credit
            $this->addJournalItem($j1, '1099', $amount, 0); // Debit Clearing
            
            // 2. Dest Journal: Dr DestAccount, Cr '1099' (Funds in Transit).
            ...
            $this->addJournalItem($j2, '1099', 0, $amount); // Credit Clearing

            // If account 1099 doesn't exist, we should seed it.
    ...
    private function addJournalItem($journalId, $accountCode, $debit, $credit) {
        ...
        if (!$account) {
            ...
            if ($accountCode == '1099') {
                $this->accountModel->insert(['code'=>'1099', 'name'=>'Funds In Transit', 'type'=>'ASSET', 'normal_balance'=>'DEBIT']);
                $account = $this->accountModel->where('code', '1099')->first();
            } else {
