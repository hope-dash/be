...
<div class="space-y-5">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h3 class="font-bold text-gray-900">Domain</h3>
            <button onclick="document.getElementById('createDomainModal').classList.remove('hidden')" class="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg transition-all duration-200 bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.5v15m7.5-7.5h-15"/></svg>
                Tambah Domain
            </button>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody id="domainTableBody">
                    ...
                    <tr id="noDomainsRow"><td colspan="4" class="px-5 py-8 text-center text-gray-400">Belum ada domain.</td></tr>
                    <?php else: foreach ($domains as $d): ?>
                    <tr id="domain-row-<?= $d['id'] ?>" class="border-t border-gray-100 hover:bg-indigo-50/40">
                        <td class="px-5 py-4 font-mono text-sm"><?= esc($d['domain']) ?></td>
                        <td class="px-5 py-4"><span class="inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-semibold tracking-wide <?= $d['type'] === 'admin' ? 'bg-blue-50 text-blue-700' : ($d['type'] === 'shop' ? 'bg-emerald-50 text-emerald-700' : 'bg-purple-50 text-purple-700') ?>"><?= $d['type'] ?></span></td>
                        <td class="px-5 py-4"><span class="inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-semibold tracking-wide <?= $d['is_verified'] ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700' ?>"><?= $d['is_verified'] ? 'Verified' : 'Pending' ?></span></td>
                        <td class="px-5 py-4 text-right">
                            <button onclick="deleteDomain(<?= $d['id'] ?>)" class="px-2.5 py-1.5 text-xs font-semibold rounded-lg text-red-600 hover:text-red-700 hover:bg-red-50">Hapus</button>
                        </td>
                    </tr>
...
<!-- Modal Create Domain -->
<div id="createDomainModal" class="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm hidden flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6" onclick="event.stopPropagation()">
        <div class="flex items-center justify-between mb-5">
            <h3 class="text-lg font-bold text-gray-900">Tambah Domain</h3>
            <button onclick="document.getElementById('createDomainModal').classList.add('hidden')" class="p-2 rounded-xl hover:bg-gray-100 transition-colors">
                <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>
        <form id="domainForm" class="space-y-4">
            <div id="domainError" class="hidden bg-red-50 border border-red-200 text-red-700 rounded-xl px-4 py-3 text-sm mb-4"></div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Domain *</label>
                <input type="text" name="domain" class="w-full rounded-xl border-2 border-gray-200 px-4 py-2.5 text-sm outline-none transition-all bg-white hover:border-gray-300 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50" placeholder="contoh.satualur.my.id" required>
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Tipe *</label>
                <select name="type" class="w-full rounded-xl border-2 border-gray-200 px-4 py-2.5 text-sm outline-none transition-all bg-white hover:border-gray-300 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50" required>
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm">Simpan</button>
                <button type="button" onclick="document.getElementById('createDomainModal').classList.add('hidden')" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl border-2 border-gray-200 text-gray-600 hover:bg-gray-50">Batal</button>
            </div>
        </form>
    </div>
</div>
...
document.getElementById('domainForm').addEventListener('submit', function(e) {
    ...
    fetch('/cms/tenants/<?= $tenant['id'] ?>/domains/create', { method: 'POST', body: formData })
    .then(r => r.json())
    .then(data => {
        if (data.status) {
            location.reload();
...
function deleteDomain(domainId) {
    ...
    fetch('/cms/tenants/<?= $tenant['id'] ?>/domains/' + domainId + '/delete', { method: 'POST' })
    .then(r => r.json())
    .then(data => {
        if (data.status) {
            ...
            if (tbody && tbody.querySelectorAll('tr[id^=\"domain-row\"]').length === 0) {
                tbody.innerHTML = '<tr id=\"noDomainsRow\"><td colspan=\"4\" class=\"px-5 py-8 text-center text-gray-400\">Belum ada domain.</td></tr>';
            }
        } else {
            alert(data.message);
        }
